...
import {FlaskConical,ListFilter,Play,Save} from 'lucide-react';
...
export default function App(){
  const [tab,setTab]=useState<'judgments'|'search'>('search');
  return <main className="shell"><header className="topbar"><FlaskConical size={20}/><strong>Search Relevance Lab</strong><nav className="tabs"><button className={tab==='search'?'on':''} onClick={()=>setTab('search')}><ListFilter size={15}/>搜索工作台</button><button className={tab==='judgments'?'on':''} onClick={()=>setTab('judgments')}>Judgments</button></nav><small>Local workspace</small></header>{tab==='search'?<SearchWorkbench/>:<Judgments/>}</main>;
}
